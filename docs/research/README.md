# Research Notes

Personal learning journey notes for understanding and building language models from first principles.

## Purpose

This folder contains:
- **Implementation notes** from working through educational codebases (microGPT, nanoGPT, etc.)
- **Trend tracking** for keeping up with LLM research and tools
- **Experiment logs** documenting what worked and what didn't

## Structure

```
docs/research/
├── README.md           # This file
├── microgpt_notes.md   # microGPT/nanoGPT learning notes
└── trends_links.md     # Living watchlist of papers, talks, tools
```

## Update Cadence

| Doc | Frequency | Trigger |
|-----|-----------|---------|
| `microgpt_notes.md` | After each learning session | Completing a checkpoint or experiment |
| `trends_links.md` | Weekly/bi-weekly | Discovering high-signal content |

**Tip:** Keep entries dated and brief. Use TODO markers for things to revisit.

## Quick Links

- [microGPT Learning Notes](./microgpt_notes.md)
- [Trends & Links Watchlist](./trends_links.md)
